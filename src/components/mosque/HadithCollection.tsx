...
const HadithCollection = () => {
  ...
  const loadChapters = useCallback(async (book: Book) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/${book.bookSlug}/chapters?apiKey=${API_KEY}`);
  ...
  const loadHadiths = useCallback(async (bookSlug: string, chapter?: Chapter, page = 1, search?: string) => {
    ...
    setError("");
    try {
      let url = `${API_BASE}/hadiths/?apiKey=${API_KEY}&book=${bookSlug}&paginate=${PAGE_SIZE}&page=${page}`;
      if (chapter) {
        url += `&chapter=${chapter.chapterNumber}`;
      }
      if (search && search.trim()) {
        url += `&hadithEnglish=${encodeURIComponent(search.trim())}`;
      }
  ...
  const copyHadith = (h: HadithItem) => {
    const parts = [];
    if (h.hadithArabic) parts.push(h.hadithArabic);
    if (h.englishNarrator) parts.push(`${h.englishNarrator}:`);
    if (h.hadithEnglish) parts.push(h.hadithEnglish);
    if (h.hadithUrdu) parts.push(h.hadithUrdu);
    ...
    <section ref={topRef} id="hadith-collection" className="px-3 sm:px-4 max-w-4xl mx-auto">
      ...
      {error && !loading && (
        <div className="glass-card p-6 text-center mb-4">
          ...
          <button onClick={() => { setError(""); loadBooks(); }} className="px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-body">
            Retry
          </button>
        </div>
      )}
              ...
              {books.map((book) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] sm:text-xs text-muted-foreground/70 font-body mt-0.5">
                      {book.writerName}
                    </p>
                    <p className="text-[10px] sm:text-xs text-muted-foreground/50 font-body mt-0.5">
                      {book.hadiths_count?.toLocaleString()} hadiths • {book.chapters_count} chapters
                    </p>
                  </div>
        ...
        <div className="glass-card overflow-hidden">
          <div className="gradient-primary px-4 py-4 sm:px-6 sm:py-5">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-primary-foreground/60 font-body mt-0.5">
                  {selectedBook.writerName} • {chapters.length} chapters
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="px-4 py-3 sm:px-6 border-b border-border">
            <div className="flex items-center gap-2">
              ...
              <button onClick={handleSearch} className="px-4 py-2.5 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition-colors text-sm font-body font-medium shrink-0">
                Search
              </button>
            </div>
          </div>
              ...
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="font-body text-sm sm:text-base text-foreground font-medium group-hover:text-primary transition-colors line-clamp-2">
                      {ch.chapterEnglish}
                    </p>
                    {ch.chapterUrdu && (
                      <p className="font-urdu text-xs text-muted-foreground mt-1 line-clamp-1" dir="rtl">
                        {ch.chapterUrdu}
                      </p>
                    )}
                  </div>
        ...
        <div className="space-y-4">
          ...
          <div className="glass-card overflow-hidden">
            <div className="gradient-primary px-4 py-3 sm:px-6 flex items-center gap-2 text-xs text-primary-foreground/80 font-body flex-wrap">
              <button onClick={() => { setView("books"); setSelectedBook(null); setChapters([]); setHadiths([]); setError(""); setSearchQuery(""); }} className="hover:text-primary-foreground transition-colors whitespace-nowrap">
                Books
              </button>
            ...
            <div className="px-4 py-3 sm:px-6 sm:py-4 bg-secondary/30 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-heading text-sm sm:text-base font-bold text-foreground truncate">
                  {selectedChapter?.chapterEnglish || `Search: "${searchQuery}"`}
                </h3>
                <p className="text-[10px] sm:text-xs text-muted-foreground font-body mt-0.5">
                  {selectedBook?.bookName} • {totalHadiths} hadiths
                </p>
              </div>
            </div>
          </div>
            ...
            <div key={h.id} className="glass-card overflow-hidden">
              ...
              <div className="flex items-center justify-between px-4 sm:px-6 py-3 bg-primary/5 border-b border-border/30">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => copyHadith(h)} className="p-2 rounded-lg hover:bg-secondary text-muted-foreground/40 hover:text-primary transition-colors">
                    <Copy className="w-4 h-4" />
                  </button>
                  <button onClick={() => shareHadith(h)} className="p-2 rounded-lg hover:bg-secondary text-muted-foreground/40 hover:text-primary transition-colors">
              ...
              {h.hadithArabic && (
                <div className="px-4 sm:px-6 py-5 sm:py-6 bg-primary/[0.03] border-b border-border/20">
                  <p className="font-arabic text-lg sm:text-xl text-foreground/90 text-right" dir="rtl" style={{ lineHeight: "2.6" }}>
                    {stripHtml(h.hadithArabic)}
                  </p>
                </div>
              )}
              ...
              {h.hadithEnglish && (
                <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-border/20">
                  ...
                  {h.englishNarrator && (
                    <p className="text-sm font-body font-semibold text-primary mb-2" style={{ lineHeight: "1.8" }}>
                      {stripHtml(h.englishNarrator)}
                    </p>
                  )}
                  <p className="text-sm sm:text-base text-foreground/85 font-body" style={{ lineHeight: "2" }}>
                    {stripHtml(h.hadithEnglish)}
                  </p>
                </div>
              )}
              ...
              {h.hadithUrdu && (
                <div className="px-4 sm:px-6 py-5 sm:py-6 bg-accent/[0.04]">
                  ...
                  <p className="font-urdu text-base sm:text-lg text-foreground/85 text-right" dir="rtl" style={{ lineHeight: "3", wordSpacing: "2px" }}>
                    {stripHtml(h.hadithUrdu)}
                  </p>
                </div>
              )}
            </div>
          ...
          {totalPages > 1 && (
            <div className="glass-card flex items-center justify-between px-4 py-3 sm:px-6 sm:py-4">
              <button
                onClick={() => loadHadiths(selectedBook!.bookSlug, selectedChapter || undefined, currentPage - 1, searchQuery || undefined)}
